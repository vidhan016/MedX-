...
const PatientListCard: React.FC<PatientListCardProps> = ({
  patient: p,
  isFirst = false,
  showConsult = false,
  onConsult,
  ...
  return (
    <div className={cardClass}>
      <div className={styles.pcListRow}>
        ...
        <div className={styles.pcLeftBlock}>
          <div className={styles.pcAvatarRow}>
            ...
            <div className={styles.pcInfo}>
              ...
              {/* Row 3 — Age / gender / blood / emergency */}
              ...
              {/* Row 4 — Flag + nature badges */}
              {(p.flag || natureBadges.length > 0) && (
                <div className={styles.badgeRow}>
                  {p.flag && <FlagBadge flag={p.flag} />}
                  {natureBadges.length > 0 && <NatureBadges badges={natureBadges} maxVisible={1} />}
                </div>
              )}
            </div>
          </div>
        ...
        {/* ── MIDDLE: Date + time + visit type + dx + allergies ── */}
        <div className={styles.pcMidBlock}>
          ...
          <div className={styles.pcMeta} style={{ flexWrap: "nowrap" }}>
            ...
            <span className={styles.dot}>·</span>
            <span className={styles.pcMetaItem}>
              <ClockIcon />{p.appointmentTime} – {addHalfHour(p.appointmentTime)}
            </span>
            {isFirst && (
          ...
          {/* Visit + appt type badges */}
          <div className={styles.badgeRow} style={{ marginTop: 2 }}>
            <span className={`${styles.inlineBadge} ${styles.badgeVisit}`}>
              <ScopeIcon />{p.visitType}
            </span>
          ...
          {p.allergies && p.allergies.length > 0 && (
            <div className={styles.badgeRow} style={{ marginTop: 2 }}>
              <AllergyBadges allergies={p.allergies} maxVisible={1} />
            </div>
          )}
        </div>

        {/* ── VITALS ── */}
        <VitalsGrid vitals={p.vitals} />

        {/* ── ACTIONS ── */}
        <div className={styles.pcActions}>
          {showConsult && (
            <button className={styles.consultBtn} onClick={() => onConsult?.(p)}>
              <ScopeIcon /> Consult
            </button>
          )}
          <ActionMenu patientId={p.id} items={menuItems} upward={false} />
        </div>
      </div>
    </div>
  );
