...
const Avatar: React.FC<AvatarProps> = ({
  ...
  // ── Inline style for custom size + bg ──
...
export interface AvatarGroupProps {
  ...
  /** Controls overlap spacing */
  groupSize?: AvatarGroupSize;
  /** Max avatars to show before a "+N" overflow badge */
  max?: number;
  /** Extra class names */
  className?: string;
}
...
const AvatarGroup: React.FC<AvatarGroupProps> = ({
  ...
  return (
    <div className={groupClass}>
      ...
      {overflow > 0 && (
        <div
          className="avatar avatar-md rounded-full overflow-hidden"
          style={{ marginLeft: groupSize === "xs" ? "-0.35rem" : groupSize === "sm" ? "-0.5rem" : "-0.75rem" }}
        >
          <div
            className="avatar-title text-xs font-semibold select-none rounded-full"
            style={{ background: "var(--bs-secondary-bg-subtle)", color: "var(--bs-secondary)" }}
          >
            +{overflow}
          </div>
        </div>
