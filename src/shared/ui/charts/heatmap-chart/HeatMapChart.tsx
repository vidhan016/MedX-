...
const HeatMapChart: React.FC<HeatMapChartProps> = ({ data = [], queryKey, queryFn }) => {
              ...
              <div key={wi} className="hmCol">
                {Array.from({ length: 7 }, (_, di) => {
                  ...
                  return (
                    ...
                    >
                      <span className="hmNum" style={{ color: getTextColor(d.val, max) }}>
                        {d.val > 0 ? d.val : ""}
                      </span>
                      {hov === idx && d.val > 0 && (
                        <div className="hmTooltip">
                          <div className="hmTooltipDay">{DAY_LABELS[di]}, W{wi + 1}</div>
...
// ── Re-export type alias so existing imports don't break ───────────────────────
